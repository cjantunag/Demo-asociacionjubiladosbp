...
function RAD_focusNextField(seln) {
 ...
 for(var i=0;i<document.F.elements.length; i++) {
  if (document.F[i].name==selFieldName) {
   ...
   if (selFieldName2.length>0 && document.F[k].type!='radio') { eval('document.F.'+selFieldName2+'.focus();'); if (document.F[k].type=='text') eval('document.F.'+selFieldName2+'.select();'); }
...
var wh=window.open(pagina,"_blank","width=0,height=0,resizable=yes,scrollbars=yes,status=no,toolbar=no,menubar=no,titlebar=no");
if (String(x)!='undefined' && String(y)!='undefined') wh.resizeTo(x,y);
else wh.resizeTo(screen.width,screen.height);
}

//	written	by Tan Ling	Wee	on 2 Dec 2001 nu
