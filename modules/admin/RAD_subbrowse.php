...
if ($func=="edit" || $func=="new" || $func=="detail") {
	if ($fields[$i]->browsedit && $subfunc!="list") {
    	    $TMP_URLfileNew=$TMP_DEF_URL."index.php?V_dir=$TMP_dir&V_mod=".$TMP_file."&V_idmod=$TMP_idmod&V_prevdir=$V_dir&V_prevmod=$V_mod&menuoff=x&headeroff=x&footeroff=x&func=new&subfunc=browse&V_roi=".urlencode($TMP_field1."='".$TMP_id."'").$SESSION_SID.$TMP_V_roi2;
    	    $TMP_URLfileNew=$PHP_SELF."?V_dir=$TMP_dir&V_mod=".$TMP_file."&V_idmod=$TMP_idmod&V_prevdir=$V_dir&V_prevmod=$V_mod&menuoff=x&headeroff=x&footeroff=x&func=new&subfunc=browse&V_roi=".urlencode($TMP_field1."='".$TMP_id."'").$SESSION_SID.$TMP_V_roi2;
