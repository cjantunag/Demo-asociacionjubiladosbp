...
if(!isset($Add_Col)) $Add_Col = "";
if(!isset($Add_Row)) $Add_Row = "";
if(!isset($Rows)) $Rows = "";
if(!isset($InsCol)) $InsCol = "";
if(!isset($DelCol)) $DelCol = "";
if(!isset($Criteria)) $Criteria = "";
if(!isset($InsRow)) $InsRow = "";
if(!isset($DelRow)) $DelRow = "";
if(!isset($AndOrRow)) $AndOrRow = "";
if(!isset($AndOrCol)) $AndOrCol = "";
...
while($i < $num_tables) {
    $tbl = $A_tables[$i];
    $num_fields=0;
    $fld_results = sql_list_fields($tbl, $RAD_dbi);
...
?>
	...
	<input type="hidden" name="func" value="tbl_qbe">
...
for($x = 0; $x < $col; $x++) {
    ...
    if(isset($DelCol[$x]) && $DelCol[$x]== "on") continue;
    echo "<td align=center bgcolor='$cfgBgcolorOne'><select style='width: $widem;' name='Field[$z]' size='1'>\n";
    echo "<option value=''></option>\n";
    for($y = 0; $y < sizeof($fld); $y++) {
        $sel =  "";
...
for($x = 0; $x < $col; $x++) {
    ...
    echo  "<option $sel value='DESC'>Descending</option>\n";
    echo  "</select>\n";
    echo  "</td>\n";
    $z++;
}
...
?>
	...
	<input type="hidden" name="table" value="<?=$table?>">
<?
echo  "<input type='hidden' name='subfunc' value='db_details'>\n";
echo  "<input type='hidden' name='zero_rows' value='Your SQL-query has been executed successfully'>";
echo  "<input ACCESSKEY='S' TITLE='ALT+S' type='submit' name='SQL' value='Submit Query'></td></tr></table></td><td>";
