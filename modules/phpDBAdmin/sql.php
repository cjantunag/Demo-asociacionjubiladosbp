<?
global $PHPSESSID, $PHP_SELF, $SESSION_SID, $RAD_dbi;
if (eregi(basename(__FILE__), $PHP_SELF)) die ("Security Error ...");

if(isset($subfunc) && $subfunc == "sql") {
    $page = "index.php?V_dir=$V_dir&V_mod=$V_mod&func=sql&db=$db&table=$table&pos=$pos&sql_query=".urlencode($sql_query).$SESSION_SID;
} else if($subfunc == "") {
    $page = "index.php?V_dir=$V_dir&V_mod=$V_mod&db=$db".$SESSION_SID;
}

// Go back to further page if table should not be dropped
if(isset($btnDrop) && $btnDrop == $strNo) {
    if ($page=="") $page=$PHP_SELF."?V_dir=$V_dir&V_mod=$V_mod&func=db_details";
...
//elseif (!$is_drop_sql_query || $btnDrop == $strYes)
    ...
    if(isset($sessionMaxRows)) $cfgMaxRows = $sessionMaxRows;
    $sql_limit = (isset($pos) && eregi("^SELECT", $sql_query)) ? " LIMIT $pos, $cfgMaxRows" : '';
    if (strtolower(_DEF_dbtype)=="oracle") $sql_limit = "";
    $result = sql_query($sql_query.$sql_order.$sql_limit, $RAD_dbi);
    // the same SELECT without LIMIT
    if(eregi("^SELECT", $sql_query)) {
        $OPresult = sql_query($sql_query.$sql_order, $RAD_dbi);
        $SelectNumRows = @sql_num_rows($OPresult);
    }

    if(!$result) {
        $error = $sql_query.$sql_order.$sql_limit.sql_error();
        include("modules/$V_dir/header.inc.php");
        die($error);
    }

    $num_rows = sql_num_rows($result);
    if (strtolower(_DEF_dbtype)=="oracle") $num_rows=1;
    ...
    if($num_rows < 1) {
        ...
        if(!eregi("SHOW VARIABLES|SHOW PROCESSLIST|SHOW STATUS", $sql_query))
            echo "<p><a href=\"?V_dir=$V_dir&V_mod=$V_mod&func=tbl_change&db=$db&table=$table&pos=$pos&subfunc=$subfunc&sql_query=".urlencode($sql_query).$SESSION_SID."\"> $strInsertNewRow</a></p>";
    }
}
