...
<a href='#' onclick=\"document.getElementById('captcha').src='".$dir_root."modules/securimage/securimage_show.php?'+Math.random();return false\"><img src='".$dir_root."images/reload.gif' title='Recarga Imagen' alt='Recarga Imagen' border=0></a>";
	...
	if ($cmd=="slide") {
		...
		$TMP_row["contenido_".$lang]=str_replace("<!-- securimage -->",$TMP_secu,$TMP_row["contenido_".$lang]);
		$TMP_row["contenido_".$lang]=str_replace("< script","<script",$TMP_row["contenido_".$lang]);
		$TMP_id = $TMP_row[id];
...
//				$barra=$ant.$title;
	...
	$TMP_row["contenido_".$lang]=str_replace("<!-- securimage -->",$TMP_secu,$TMP_row["contenido_".$lang]);
	$TMP_row["contenido_".$lang]=str_replace("< script","<script",$TMP_row["contenido_".$lang]);
	//Si es un articulo que contiene a otro artículo => no mostrar el icono de print: variable que lo controlara
...
function nuevoarticulo($id) {
	...
	if ($id=="") {
		...
		while($TMP_row = sql_fetch_array($result, $RAD_dbi)) {
			...
			echo "<td><a href=\"".$dir_root."index.php?V_dir=$V_dir&V_mod=$V_mod&id=".$TMP_row[id]."$SESSION_SID\">Ver Maqueta</a></td>";
			echo "<td><a href=\"javascript:RAD_OpenW('".$dir_root."index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$TMP_row[id].$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='".$dir_root."images/new.gif' border='0' alt='nueva pagina'> Crear Nueva P&aacute;gina </a></td></tr>";
		}
		echo "<tr><td>P&aacute;gina sin maqueta (contenido libre)</td>";
		echo "<td>&nbsp;</td>";
		echo "<td><a href=\"javascript:RAD_OpenW('".$dir_root."index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=&headeroff=x&footeroff=x".$SESSION_SID."');\"> <img src='".$dir_root."images/new.gif' border='0' alt='nueva pagina'> Crear Nueva P&aacute;gina </a></td></tr>";
		echo "</table>";
	} else {
		editaarticulo("", $id);
	}
