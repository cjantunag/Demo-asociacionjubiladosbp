...
function listaarticulos($TMP_secid,$TMP_idlis,$TMP_idartparent,$TMP_level) {
	...
	if ($owner=="" && $TMP_idartparent=="" && $TMP_idlis=="") {
		...
		echo "\n<table border=0 cellpadding=0 cellspacing=0 align=center width=100%>";
		echo "<tr><th><b>Mis P�ginas</b></th></tr>";
		echo "\n</table>";
	}
	...
	if (is_admin() || is_editor() || is_viewer()) {
		...
		if ($color=="style='background-color:white'") $color="style='background-color:#E8E8E0'";
			...
			else echo "<tr><td $color align=left> ".$TMP_x."<a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=".$TMP_id.$SESSION_SID."\"> <img src='images/edit.gif' border='0' alt='edita p�gina'> $title </a> [".$nombreautor."]";
...
function nuevoarticulo($id) {
	...
	if (!is_user()) {
		echo "<b><center>Se necesita ser usuario para acceder a esta funci�n</b></center>";
		CloseTable();
		die;
	}
	if ($id=="") {
		echo "<b><center>Selecciona Maqueta para crear P�gina</b></center><br><ul><table border=0>";
		...
		while($TMP_row = sql_fetch_array($result, $RAD_dbi)) {
			...
			echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$TMP_row[id].$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='images/new.gif' border='0' alt='nueva p�gina'> Crear Nueva P�gina </a></td></tr>";
		}
		echo "<tr><td>P�gina sin maqueta (contenido libre)</td>";
		echo "<td>&nbsp;</td>";
		echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=&headeroff=x&footeroff=x".$SESSION_SID."');\"> <img src='images/new.gif' border='0' alt='nueva p�gina'> Crear Nueva P�gina </a></td></tr>";
		echo "</table>";
	} else {
		editaarticulo("", $id);
	}
...
function editaarticulo($id, $maquetaid) {
	...
	if ($id!="") {
		...
		if (!is_admin() && !is_editor() && ($autor!=$user || $user=="") ) {
			echo "<b><center>Solo el autor puede editar la p�gina</b></center>";
			CloseTable();
			die;
		}
	} else if ($maquetaid!="") {
...
//	if ($maquetaid=="" && $id>0 && ($cmd=="new" || $cmd=="edit")) {
	...
	$formulario .= "<tr><TD width=30% class=detailtit>Secci�n : </td><TD class=detail>".RAD_editfield("V0_idseccion", "plistdb", 12, 12, "articulossecciones:id:nombre", "", false, $idseccion, "")."</td></tr>";
	$formulario .= "<tr><td class=detailtit>P�gina Padre : </td><td class=detail>".RAD_editfield("V0_idartparent", "popupdb", 45, 45, "articulos:id:nombre", "", true, $idartparent, "")."</td></tr>";
	$formulario .= "<tr><td class=detailtit>T�tulo : </td><td class=detail>".RAD_editfield("V0_title", "string", 64000, 60, "", "", true, $title, "")."</td></tr>";
	...
	$formulario .= "<tr><td class=detailtit>P�blico : </td><td class=detail>".RAD_editfield("V0_publico", "rlist", 1, 1, "1:Si,0:No", "", false, $publico, "")."</td></tr>";
