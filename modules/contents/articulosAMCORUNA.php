...
function listaarticulos($TMP_secid,$TMP_idlis,$TMP_idartparent,$TMP_level) {
	...
	if (is_user()) echo "<tr><td align=right><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='images/new.gif' border='0' alt='nueva pagina'> Crear Nuevo </a></td></tr>";
	if (is_admin() || is_editor() || is_viewer()) {
		...
		if ($color=="style='background-color:white'") $color="style='background-color:#E8E8E0'";
			...
			else echo "<tr><td $color align=left> ".$TMP_x."<a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=".$TMP_id.$SESSION_SID."\"> <img src='images/edit.gif' border='0' alt='edita pagina'> $title </a> [".$nombreautor."]";
...
//xx	echo $arts;
	...
	if ($noprint=="")
        ...
        if ($cmd=="print") echo "\n<script>\nwindow.print();\n</script>\n";
	if (is_admin() || is_editor() || ($autor==$user && $autor!="" && $user!="")) {
		echo "&nbsp;|&nbsp;<a href=\"javascript:RAD_OpenW('index.php?V_dir=AMCORUNA&V_mod=admarticulos&cmd=edit&noeditamaqueta=x&par0=$id&func=edit&headeroff=x&footeroff=x".$SESSION_SID."');\"><img src=\"images/edit.gif\" border=\"0\" Alt=\""._EDIT."\"> "._EDIT."</a>";
		if ($esmaqueta==true && $noeditamaqueta=="") echo "&nbsp;|&nbsp;<a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dirV_mod=$V_mod&cmd=edit&id=$id&headeroff=x&footeroff=x".$SESSION_SID."');\"><img src=\"images/edit.gif\" border=\"0\" Alt=\""._EDIT." con Maqueta\"> "._EDIT." con Maqueta</a>";
	}
	echo "<br></center>";
...
//		</td></tr></table>\n";
	...
	*/
	...
	echo '<div id="contenedor_imprime">';
	echo '<h1>'.$title.'</h1>';
	echo '<p>'.$content.'</p>';
	echo '</div>';
...
function nuevoarticulo($id) {
	...
	if ($id=="") {
		...
		while($TMP_row = sql_fetch_array($result, $RAD_dbi)) {
			...
			echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$TMP_row[id].$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='images/new.gif' border='0' alt='nueva pagina'> Crear Nueva P&aacute;gina </a></td></tr>";
		}
		echo "<tr><td>P&aacute;gina sin maqueta (contenido libre)</td>";
		echo "<td>&nbsp;</td>";
		echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=&headeroff=x&footeroff=x".$SESSION_SID."');\"> <img src='images/new.gif' border='0' alt='nueva pagina'> Crear Nueva P&aacute;gina </a></td></tr>";
		echo "</table>";
	} else {
		editaarticulo("", $id);
	}
...
//	if ($maquetaid=="" && $id>0 && ($cmd=="new" || $cmd=="edit")) {
	...
	if ($esmaqueta==false && $maquetaid=="" && ($cmd=="new" || $cmd=="edit")) {
		...
		global $owner;
		$owner="x";
		include_once ("modules/AMCORUNA/admarticulos.common.php");
		include_once ("modules/AMCORUNA/admarticulos.php");
		return;
	}
...
//	OpenTable();
	...
	if ($id!="") {
		...
		if (($oldvisible=="0"||$oldpublico=="0"||$oldvisible==""||$oldpublico=="") && ($V0_publico!="0" && $V0_visible!="0")) {
			...
			$TMP_mensaje.="<a href='"._DEF_URL."index.php?V_dir=contents&V_mod=$V_mod&id=$id'>";
			$TMP_mensaje.=_DEF_URL."index.php?V_dir=contents&V_mod=$V_mod&id=$id</a>\n";
		...
		if ($V0_publico!="0" && $V0_visible!="0") {
			...
			$TMP_mensaje.="<a href='"._DEF_URL."index.php?V_dir=contents&V_mod=$V_mod&id=$TMP_id'>";
			$TMP_mensaje.=_DEF_URL."index.php?V_dir=contents&V_mod=$V_mod&id=$TMP_id</a>\n";
			$TMP_mensaje=str_replace("'","\"",$TMP_mensaje);
			if ($NOMAILCHANGES=="") RAD_sendMail(_DEF_ADMINMAIL,"","","","Articulo puesto como Visible y Publico",$TMP_mensaje,"");
		}
	}
