...
function listaarticulos($TMP_secid,$TMP_idlis,$TMP_idartparent,$TMP_level) {
	global $RAD_dbi, $user, $SESSION_SID, $owner, $secid, $CondPublico, $V_dir, $V_mod, $A_ART_LISTADOS, $id, $color, $headeroff, $footeroff, $noprint;
	...
	if ($owner=="" && $TMP_idartparent=="" && $TMP_idlis=="") {
		...
		echo "\n<table border=0 cellpadding=0 cellspacing=0 align=center width=100%>";
		echo "<tr><th><b>Mis Páginas</b></th></tr>";
		echo "\n</table>";
	}
	...
	while (list($TMP_id, $TMP_secid, $idartparent, $title, $content, $counter, $autor) = sql_fetch_row($result, $RAD_dbi)) {
		...
		echo "</td>";
		if ($noprint=="" && (is_admin() || is_editor())) echo "<td $color><a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=print&id=$TMP_id$Xowner\" target=_blank><img src=\"images/print.gif\" border=\"0\" Alt=\""._PRINT."\"> "._PRINT."</a></td>";
	...
	if ($TMP_cont==0 && $TMP_idartparent=="" && $TMP_idlis=="") echo "<tr><td $color> No hay artículos en esta Sección.</td></tr>";

	if ($owner!="" || $secid!="") echo "<tr><td align=center> [ <a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod\">"._SECTIONS." </a> ]</td></tr>";
	if (is_user()) echo "<tr><td align=right><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='images/new.gif' border='0' alt='nueva página'> Crear Nuevo </a></td></tr>";
	if (is_admin() || is_editor() || is_viewer()) {
		...
		if ($color=="style='background-color:white'") $color="style='background-color:#E8E8E0'";
			...
			else echo "<tr><td $color align=left> ".$TMP_x."<a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=".$TMP_id.$SESSION_SID."\"> <img src='images/edit.gif' border='0' alt='edita página'> $title </a> [".$nombreautor."]";
...
function muestraarticulo($id, $page) {
	global $lang, $RAD_dbi, $user, $SESSION_SID, $owner, $CondPublico, $V_dir, $V_mod, $cmd, $headeroff, $footeroff, $dbname, $showdocs, $noeditamaqueta, $noprint, $onlyshowcontenido;
	if ($dbname=="") $dbname=_DEF_dbname;
...
//	if ($title!="") echo "<center><b>$title</b></center>";
...
//xx	echo $arts;
	...
	if ($cmd=="" && $noprint=="") echo "<a href=\"index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=print&id=$id$Xowner\" target=_blank><img src=\"images/print.gif\" border=\"0\" Alt=\""._PRINT."\"> "._PRINT." </a>";
	if ($cmd=="print") echo "\n<script>\nwindow.print();\n</script>\n";
	...
	if (is_admin() || is_editor() || ($autor==$user && $autor!="" && $user!="") ) {
		echo "&nbsp;|&nbsp;<a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&noeditamaqueta=x&id=$id&headeroff=x&footeroff=x".$SESSION_SID."');\"><img src=\"images/edit.gif\" border=\"0\" Alt=\""._EDIT."\"> "._EDIT."</a>";
		if ($esmaqueta==true && $noeditamaqueta=="") echo "&nbsp;|&nbsp;<a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=$id&headeroff=x&footeroff=x".$SESSION_SID."');\"><img src=\"images/edit.gif\" border=\"0\" Alt=\""._EDIT." con Maqueta\"> "._EDIT." con Maqueta</a>";
	}
	echo "<br></center>";
...
function nuevoarticulo($id) {
	...
	if (!is_user()) {
		echo "<b><center>Se necesita ser usuario para acceder a esta función</b></center>";
		CloseTable();
		die;
	}
	if ($id=="") {
		echo "<b><center>Selecciona Maqueta para crear Página</b></center><br><ul><table border=0>";
		...
		while($TMP_row = sql_fetch_array($result, $RAD_dbi)) {
			...
			echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=new&id=".$TMP_row[id].$SESSION_SID."&headeroff=x&footeroff=x');\"> <img src='images/new.gif' border='0' alt='nueva página'> Crear Nueva Página </a></td></tr>";
		}
		echo "<tr><td>Página sin maqueta (contenido libre)</td>";
		echo "<td>&nbsp;</td>";
		echo "<td><a href=\"javascript:RAD_OpenW('index.php?V_dir=$V_dir&V_mod=$V_mod&cmd=edit&id=&headeroff=x&footeroff=x".$SESSION_SID."');\"> <img src='images/new.gif' border='0' alt='nueva página'> Crear Nueva Página </a></td></tr>";
		echo "</table>";
	} else {
		editaarticulo("", $id);
	}
...
function editaarticulo($id, $maquetaid) {
	global $RAD_dbi, $user, $dbname, $CondPublico, $V_dir, $V_mod, $cmd, $PHPSESSID, $PHP_SELF, $noeditamaqueta;
	...
	if ($id!="") {
		...
		if (!is_admin() && !is_editor() && ($autor!=$user || $user=="") ) {
			echo "<b><center>Solo el autor puede editar la página</b></center>";
			CloseTable();
			die;
		}
	} else if ($maquetaid!="") {
...
//	if ($maquetaid=="" && $id>0 && ($cmd=="new" || $cmd=="edit")) {
	...
	$formulario .= "<tr><TD width=30% class=detailtit>Sección : </td><TD class=detail>".RAD_editfield("V0_idseccion", "plistdb", 12, 12, "articulossecciones:id:nombre", "", false, $idseccion, "")."</td></tr>";
	$formulario .= "<tr><td class=detailtit>Página Padre : </td><td class=detail>".RAD_editfield("V0_idartparent", "popupdb", 45, 45, "articulos:id:nombre", "", true, $idartparent, "")."</td></tr>";
	$formulario .= "<tr><td class=detailtit>Título : </td><td class=detail>".RAD_editfield("V0_title", "string", 64000, 60, "", "", true, $title, "")."</td></tr>";
	...
	$formulario .= "<tr><td class=detailtit>Público : </td><td class=detail>".RAD_editfield("V0_publico", "rlist", 1, 1, "1:Si,0:No", "", false, $publico, "")."</td></tr>";
...
//	OpenTable();
	...
	if ($id!="") {
		...
		if (($oldvisible=="0"||$oldpublico=="0"||$oldvisible==""||$oldpublico=="") && ($V0_publico!="0" && $V0_visible!="0")) {
			$TMP_mensaje="El usuario $TMP_user ha modificado el articulo con Titulo '$V0_title' como visible y publico.\nPara verlo se puede visitar ";
...
// Recoger fuente y permitir sustituirlo, y pedir tamaño si es necesario
