...
global $PHPSESSID, $SESSION_SID;
    global $pos, $cfgMaxRows, $server, $db, $table, $sql_query, $sql_order, $sessionMaxRows, $SelectNumRows;
...
function auth() {
    ...
    Header("WWW-authenticate: basic realm=\"MySQL on " . $cfgServer['host'] . "\"");
...
// added for RAD
    ...
    $cfgServer['host']= _DEF_dbhost;
    $cfgServer['user']= _DEF_dbuname;
    $cfgServer['password']= _DEF_dbpass;
...
function display_table ($dt_result) {
    global $cfgBorder, $cfgBgcolorOne, $cfgBgcolorTwo, $cfgMaxRows, $pos, $server, $db, $table, $sql_query, $sql_order, $cfgOrder, $cfgShowBlob;
    ...
    global $sessionMaxRows, $strGo, $strShow, $strRowsFrom, $pos_next, $SESSION_SID, $V_dir, $V_mod;
    ...
    while($field = mysql_fetch_field($dt_result)) {
        if(@mysql_num_rows($dt_result)>1)
        {
            ...
            if(!eregi("SHOW VARIABLES|SHOW PROCESSLIST|SHOW STATUS", $sql_query))
                echo "<A HREF=\"?V_dir=$V_dir&V_mod=$V_mod&func=sql&server=$server&db=$db&pos=$pos&sql_query=".urlencode($sql_query)."&sql_order=$sort_order&table=$table$SESSION_SID\">";
            echo $field->name;
            if(!eregi("SHOW VARIABLES|SHOW PROCESSLIST|SHOW STATUS", $sql_query))
                echo "</a>";
            echo "</th>\n";
...
//////
        ...
        if($sql_query == "SHOW STATUS" || $sql_query == "SHOW VARIABLES") {
            ...
            echo "<td align=right><a href='?V_dir=$V_dir&V_mod=$V_mod&func=sql&db=mysql&sql_query=".urlencode("KILL $Id").$SESSION_SID."'>KILL</a></td>\n";
	  } else {
