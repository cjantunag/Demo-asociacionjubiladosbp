...
//	    echo "</HEAD>\n<BODY onkeypress='captureKey(event)' onUnLoad='javascript:if (focusNextField==false) opener.RAD_focusNextField(\"".$searchfield."\");' onLoad='javascript:window.focus();' bgcolor=white>\n";
	...
	if (($fields[$findex[$searchfieldX]]->dtype =="fpopupdb"||$fields[$findex[$searchfieldX]]->dtype =="fbpopupdb") && $searchfieldX!="" && $searchval!="") {
	    if ($keyfield!="") {
		$WHERE=" WHERE ".$keyfield." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
		$V_roitmp="$keyfield='$searchval'";
	    }
	    else $WHERE=" WHERE ".$searchfieldX." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
	    ...
	    if ($WHERE=="") $WHERE=" WHERE $litfield LIKE '".$RAD_searchbeginlikepopupdb.$vlit."%'";
	    else $WHERE.=" AND $litfield LIKE '".$RAD_searchbeginlikepopupdb.$vlit."%'";
	...
	if ($WHERE=="" || ($param!="" && $arrtmp[1]!="")) {
	  if ($arrtmp[1]!="") {
	    if ($searchval!="") {
		if ($WHERE=="") $WHERE=" WHERE ".$pfname." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'"; // where por defecto si esta vacio
		//else $WHERE.=" AND ".$pfname." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
	    }
	    if ($param!="") {
	    ...
	    if ($searchval!="") {
		if ($WHERE=="") $WHERE=" WHERE ".$pfname." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
		else $WHERE.=" AND ".$pfname." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
	    }
	    if (($fields[$findex[$searchfieldX]]->dtype =="bpopupdb"||$fields[$findex[$searchfieldX]]->dtype =="fbpopupdb") && $vlit!="") {
		if ($WHERE=="") $WHERE=" WHERE $litfield LIKE '".$RAD_searchbeginlikepopupdb.$vlit."%'";
	...
	if ($RAD_filterpopupdb!="") {
		if ($pfilter=="") $pfilter=$RAD_filterpopupdb;
	...
	if ($porder!="") $TMP_order=" ORDER BY ".$porder;
/////////////////////////////////////////////////////////////////
////	if (empty($found)) {
		$cmdSQL="sELECT COUNT(*) FROM $ptablename".$WHERE;
		$TMP_result=sql_query($cmdSQL,$RAD_dbi);
...
//		if ($keyfield!="") $WHERE=" WHERE ".$keyfield." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
//		else $WHERE=" WHERE ".$searchfieldX." LIKE '".$RAD_searchbeginlikepopupdb.$searchval."%'";
		$cmdSQL="seLECT COUNT(*) FROM $ptablename".$WHERE;
		$TMP_result=sql_query($cmdSQL,$RAD_dbi);
	        $TMP_row=sql_fetch_array($TMP_result,$RAD_dbi);
...
//OJO	if ($limit == $rows_limit) $limit = $rows_limit*5;
	...
	$nextpage = "";
	if ($start>0) {
		//$prevpage .= '<a href='.$url.'0'.$tabURLROI.' data-role="button" data-icon="arrow-l" data-theme="b">'._DEF_NLSStart.'</a>';
		if (($start-10*$limit)>-1) {
			$prevpage .= '<a href="'.$url.($start-10*$limit).$tabURLROI.'" data-role="button" data-icon="arrow-l" data-theme="b">'._DEF_NLSPageBefore.'</a>';
		} else {
			if (($start-5*$limit)>-1) {
				$prevpage .= '<a href="'.$url.($start-5*$limit).$tabURLROI.'" data-role="button" data-icon="arrow-l" data-theme="b">'._DEF_NLSPageBefore.'</a>';
			}
		}
		$tmp = max($start-$limit,0);
		if ($tmp>0) $prevpage .= '<a href="'.$url.$tmp.$tabURLROI.'" data-role="button" data-icon="arrow-l" data-theme="b">'._DEF_NLSBefore.' </a>';
	}
	if ($start + $limit < $found) {
		$nextpage = '<a href="'.$url.($start+$limit).$tabURLROI.'" data-role="button" data-icon="arrow-r" data-theme="b">'._DEF_NLSAfter.'</a>';
		if (($start+10*$limit)<$found) {
			$nextpage .= '<a href="'.$url.($start+10*$limit).$tabURLROI.'" data-role="button" data-icon="arrow-r" data-theme="b">'._DEF_NLSPageAfter.'</a>';
		} else {
		...
		//if ($found>($start+$limit)) { $nextpage .= '<a href='.$url.($found-$limit).$tabURLROI.' data-role="button" data-icon="arrow-r" data-theme="b">'._DEF_NLSEnd.'</a>'; }
	}
...
/////////// Menu
	...
	else $cmdSQL.=$TMP_group." ORDER BY $pftitles $limitstr";
...
//if (is_admin()) echo $id."*".$pfname."*".$cmdSQL."<br>";
			...
			$value2="";
			if ($arrpftitle[1]!="") {
				if ($fields[$findex[$searchfieldX]]->dtype=="popupdbm" && $arrpftitle[1]==$pftitle) $value2="";
				else $value2 = $TMP_row[$arrpftitle[1]];
			} else {
				if ($fields[$findex[$searchfieldX]]->dtype=="popupdbm" && $pftitles==$pftitle) $value2="";
...
//OJO no porque haya valor por defecto se permite solo este	if ($fields[$findex[$searchfieldX]]->vdefault !="") {
				...
				if ($fields[$findex[$searchfieldX]]->dtype =="popupdbm") {
					...
					if ($value2==$value) echo "<A HREF=\"javascript:sel('$value','$id')\">".urldecode($value)."</A></span>";
					else echo "<A HREF=\"javascript:sel('$value $value2','$id')\">".urldecode($value)."</A></span>";
					if (count($arrpftitle)>1) {
					    for ($ki=1; $ki<count($arrpftitle); $ki++) {
					        $TMP_value= $TMP_row[$arrpftitle[$ki]];
						$TMP_value=str_replace('"','`',$TMP_value);
					        $TMP_value=str_replace('\'','`',$TMP_value);
						if ($value2==$value) echo "<span $TMP_class> - <A HREF=\"javascript:sel('$value','$id')\">$TMP_value</A></span>";
						else if ($value2!='')  echo "<span $TMP_class> - <A HREF=\"javascript:sel('$value $value2','$id')\">$TMP_value</A></span>";
					    }
					}
					...
					if ($value2==$value) echo "<A HREF=\"javascript:sel('$value','$id')\">".urldecode($value)."</A></td>";
					else echo "<A HREF=\"javascript:sel('$value $value2','$id')\">".urldecode($value)."</A></td>";
					...
					*/
					...
					if (count($arrpftitle)>1) {
						...
						if (_DEF_appname=="AT") {
						    /*
						     Mejora para AT: Cuando en los popup se requiere que se muestre
						     algun campo que contenga un identificador, relacionado con otra tabla,
						     de caracter general como pueden ser idcliente, idproveedor, idempleado, etc...
						     Buscar su literal y mostrarlo.
						     Siento tener que insertar esto en phpRAD puesto que es dificil que se
						     extrapole a otras aplicaciones diferentes de GIE pero solo son unas pocas lineas de nada ;P
						     Si es necesario me creo un RAD_js a parte, un saludo.
						    */

						    switch ($arrpftitle[$ki]) {
							case "idcliente":
							    $TMP_value=RAD_lookup("GIE_clientes","cliente","idcliente",$TMP_value);
							    break;
						    }
						}
			...
			if (($fields[$findex[$searchfieldX]]->dtype =="fpopupdb"||$fields[$findex[$searchfieldX]]->dtype =="fbpopupdb"||$fields[$findex[$searchfieldX]]->dtype =="bpopupdb")&& $found=="1") {
				echo "<script type='text/javascript'>";
				if ($fields[$findex[$searchfieldX]]->dtype =="bpopupdb" && $value!=$value2) echo "sel('$value $value2','$id');";
				else echo "sel('$value2','$id');";
				echo "</SCRIPT>\n</HTML>";
				exit;
			}
...
//			}
		...
		$func = "error";
		$errorstr .= $findex[$searchfieldX]."** ".$cmdSQL." [".sql_errorno($TMP_result)."] ".sql_error($TMP_result);
	}
}
