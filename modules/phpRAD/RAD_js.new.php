...
<?
	...
	if (_DEF_POPUP_MARGIN=="SUBMODAL") {
		echo "   var urlOpener=window.top.location.href;\n";
		echo "   if (urlOpener.indexOf('#')>0) { urlOpener=urlOpener.substr(0,urlOpener.indexOf('#')); }\n";
		echo "   window.top.location.href=urlOpener;\n";
	} else {
...
<?
    ...
    if (_DEF_POPUP_MARGIN=="SUBMODAL") echo "parent.setTimeout('RAD_focusNextField(\'".$searchfield."\');',50);  focusNextField=true;\n  parent.hidePopWin();\n";
    else echo "	parent.RAD_focusNextField('".$searchfield."');\n	focusNextField=true;\n	parent.RAD_hideL();\n";
...
//	    echo "</HEAD>\n<BODY onkeypress='captureKey(event)' onUnLoad='javascript:if (focusNextField==false) opener.RAD_focusNextField(\"".$searchfield."\");' onLoad='javascript:window.focus();' bgcolor=white>\n";
	...
	if (($fields[$findex[$searchfieldX]]->dtype =="fpopupdb"||$fields[$findex[$searchfieldX]]->dtype =="fbpopupdb") && $searchfieldX!="" && $searchval!="") {
	    if ($keyfield!="") {
		$WHERE=" WHERE ".$keyfield." LIKE '".$searchval."%'";
	...
	if ($WHERE=="" || $param!="" || $arrtmp[1]!="") {
	  if ($arrtmp[1]!="") {
	    ...
	    $TMP_result=sql_query($cmdSQL,$RAD_dbi);
	    $TMP_row=sql_fetch_array($TMP_result,$RAD_dbi);
	    if (($TMP_row[0]<1)||($TMP_row[0]==1 && $fields[$findex[$searchfieldX]]->dtype=="popupdb")) {
...
/////////// Menu
	...
	if (is_modulepermitted("", $V_dir, $ptablename)) {
		$TMP_linksearch="'modules.php?V_dir=$V_dir&V_mod=$ptablename&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=searchform&subfunc=search_js&searchfield=$searchfield&searchfieldlit=$searchfieldlit&V_roi=".urlencode($V_roitmp).$SESSION_SID."'";
		$TMP_linknew="'modules.php?V_dir=$V_dir&V_mod=$ptablename&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=new&subfunc=search_js&searchfield=$searchfield&searchfieldlit=$searchfieldlit&V_roi=".urlencode($V_roitmp).$SESSION_SID."'";
	}
	if ($TMP_mod!="" && $TMP_linksearch=="") {
		if (is_modulepermitted("", $V_dir, $TMP_mod)) {
			$TMP_linksearch="'modules.php?V_dir=$V_dir&V_mod=$TMP_mod&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=searchform&subfunc=search_js&searchfield=$searchfield&searchfieldlit=$searchfieldlit&V_roi=".urlencode($V_roitmp).$SESSION_SID."'";
			$TMP_linknew="'modules.php?V_dir=$V_dir&V_mod=$TMP_mod&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=new&subfunc=search_js&searchfield=$searchfield&searchfieldlit=$searchfieldlit&V_roi=".urlencode($V_roitmp).$SESSION_SID."'";
		}
	}
	if ($TMP_linksearch!="") {
