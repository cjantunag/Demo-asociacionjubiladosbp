...
if (($V_reportName!=""||$V_idreportName!="")&&($func=="search")) {
	...
	if ($V_idreportName=="") {
            ...
            $TMP_insert="";
            $campos = sql_list_fields(_DEF_dbname, "impresos");
            $columnas = sql_num_fields($campos);
            for ($i = 0; $i < $columnas; $i++) {$TMP_campo[] = sql_field_name($campos, $i);}

            if (in_array('modulo', $TMP_campo)) {
                $TMP_insert.=", modulo='".$V_mod."'";
            }
            if (in_array('usuario', $TMP_campo)) {
                $TMP_user=base64_decode($HTTP_SESSION_VARS["SESSION_user"]);
                $TMP_insert.=", usuario='".$TMP_user."'";
            }
...
echo "
<script>
var Vcheckuncheckdeletes=false; 
function checkuncheckdeletes(){
  if(Vcheckuncheckdeletes==true) {
				...
				if ($tablename!=$ptablename) {
					...
					//else $TMP_defaultfilter=" ON ".$tablename.".".$fields[$findex[$orderby]]->name."=".$ptablename.".".$pfname;
                                        if ($pfname=="codmunicipio" && $dbtype=="mysql") {
                                            $campos = sql_list_fields(_DEF_dbname, $tablename);
                                            $columnas = sql_num_fields($campos);
                                            for ($i = 0; $i < $columnas; $i++) {$TMP_campo[] = sql_field_name($campos, $i);}
                                        }
...
//					$cmdSQL="SELECT * FROM $ptablename JOIN $tablename ".$TMP_defaultfilter.$TMP_groupby." ORDER BY $ptablename.$pftitle ".$ASC." LIMIT $limitstr";
		...
		if ($db -> query($cmdSQL)) {
			...
			if ($func!="browsecalendar") echo '<table data-role="table" id="table-column-toggle" data-mode="columntoggle" class="ui-responsive table-stroke table-stripe" data-column-btn-text="Columnas..." data-column-btn-theme="b">';
			$browsedit=false;
		...
		$TMP_cab="<thead><tr>";
	      if ($browsetype=="editline" || $browsetype=="line" || $browsetype=="insert") $TMP_cab.="<th>#</th>";
	      if ($browsetype=="deleteline") $TMP_cab.="<th><img onclick='javascript:checkuncheckdeletes();'src='images/delete.gif' border=0 alt='"._DEF_NLSDeleteString."' title='"._DEF_NLSDeleteString."'></th>";
	      if ($RAD_CONFIRM_SAVEREG=="") $TMP_confirm_savereg="\n<script type=text/javascript>\nfunction saveregs(elem) {\n      csave=confirm(\"¿Esta seguro de guardar los registros cambiados?\");\n  if (csave) { document.forms.F.submit();\n elem.disabled=true; \n }}\n</script>\n";
	      else $TMP_confirm_savereg="\n<script type=text/javascript>\nfunction saveregs(elem) { document.forms.F.submit(); elem.disabled=true;}\n</script>\n";

// Intente hacer una funcion para duplicar una linea de insert en el fondo. No va bien porque no se enlaza adecuadamente en el DOM y da errores..

/*
function duplicarowinsert(trow){
...
*/
		...
		$TMP_confirmdeletes="	dele=confirm(\"¿Esta seguro de borrar \"+numregdel+\" registros marcados?\");\n        if (dele) {\n";
		$TMP_confirmdelete="	dele=confirm(\"¿Esta seguro de borrar este registro?\");\n        if (dele) {\n";
	      } else {
...
function savedeleregs() {
	var numregdel=0;
	var confirmstr=\"¿Esta seguro de guardar los registros cambiados?\"
	for (var j=0; j<document.F.elements.length; j++) {
		varname=document.F.elements[j].name;
		varvalue=document.F.elements[j].checked;
		if (varname.indexOf('_delete')>0 && varvalue==true) numregdel++;
        ...
        for ($i = 0; $i < $numf; $i++) {
			...
			if ($orderby == $fields[$i]->name) {
				...
				if ($subfunc!="list" && $subfunc!="report") {
					...
					$TMP_cab.='<th data-priority="'.$cont.'">'.$orderbymark.$fields[$i] -> title.'</th>';
                    $cont++;
				}
		    } else {
                $TMP_cab.='<th data-priority="'.$cont.'">'.$fields[$i]->title.' </th>';
                $cont++;
		    }
            }
        }
		$TMP_cab.='</thead></tr><tbody>';
			...
			for($ki=0; $ki<$V_rowsinsert; $ki++) {
				...
				if ($browsetype=="deleteline"||$browsetype=="editline"||$browsetype=="line"||$browsetype=="insert") $TMP_linesinsert.="<TD $RAD_classrow></TD>";
					...
					if ($fields[$i]->browsable && ($browsedit_insert) && $fields[$i]->dtype!="function") {
						...
						$TMP_linesinsert.=RAD_editfield($fields[$i]->name, $fields[$i]->dtype, $fields[$i]->length, $fields[$i]->ilength, $TMP_extra, $TMP_onchange, $fields[$i]->canbenull, $fvalue,"VI".$ki."_".$fields[$i]->name, "", $fields[$i]->vonfocus);
					    }
					    $TMP_linesinsert.="</TD>";
...
*/
		$TMP_vacio=true;
			...
			if (file_exists("modules/$V_dir/".$V_mod.".defaults.php")) {
                ...
                if ($numf==0) continue;
                $TMP_brline[$numrow].="<! RAD_browse $numrow><tr>\n";
                for ($i = 0; $i < $numf; $i++) {
                    if (${"idname$i"} != "") {
...
//			else $TMP_nextfunc="edit";
			if ($subbrowseSID!="") {
			    if ($RAD_width<2 || $RAD_width=="") $RAD_width=800;
			    if ($RAD_height<2 || $RAD_height=="") $RAD_height=600;
                	    $htmllink="<A TITLE='Ver Registro ".($numrow)."' HREF='javascript:RAD_OpenW(\"".$PHP_SELF."?func=".$TMP_nextfunc."&subfunc=browse".$urlidnames.$URLROI;
                	    $htmllinkend="</A>";
			} else {
			...
			if ($menudelete||!isset($menudelete)) $TMP_brline[$numrow].=" <a href=\"javascript:deleregx($TMP_numreg);\"><img src='images/delete.gif' border=0 alt='"._DEF_NLSDeleteString."' title='"._DEF_NLSDeleteString."'></a>";
			...
			$TMP_brline[$numrow].="<TD $RAD_classrow>".$htmlEdit;
				...
				$TMP_brline[$numrow].=RAD_editfield($fields[$i]->name, $fields[$i]->dtype, $fields[$i]->length, $fields[$i]->ilength, $TMP_extra, $fields[$i]->vonchange, $fields[$i]->canbenull, $fvalue,"V".$numrow."_".$fields[$i]->name, "", $fields[$i]->vonfocus);
			}
			if (trim($htmllink)!="") {
...
//			    $TMP_brline[$numrow].=RAD_showfield($fields[$i]->dtype, $fields[$i]->extra, $db -> Record[$fields[$i] -> name]);
			    ...
			    if ($fields[$i]->dtype!="num" && $TMP_value=="") $TMP_value="_";
				...
				if ($TMP_funclink!="" && $subfunc != "list" && $subfunc != "report") if (is_modulepermitted("", $TMP_dir, $TMP_funclink) || is_admin()) {
					if (in_array($fields[$i]->dtype,array("plistm","plistdbm","plistdbmtree","popupdbm","checkboxm","checkboxdbm"))){
					    ...
					    $TMP_arr = explode('<br>',$TMP_value);
	    				    ...
	    				    if (count($TMP_arr)>1) {
						$TMP_value="";
						foreach($TMP_arr as $TMP_idx=>$TMP_lin) $TMP_value.="<a href=\"javascript:RAD_OpenW('$PHP_SELF?V_dir=$TMP_dir&V_mod=$TMP_funclink&funclink=1&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=detail&par0=%%1%%&subfunc=browse',800,600);\"><img src='images/lupa.gif'></a> ".$TMP_lin."<br>\n";
					    } else {
						if ($TMP_value!="" && $TMP_value!="_" && $TMP_value!="&nbsp;") $TMP_value="<a href=\"javascript:RAD_OpenW('$PHP_SELF?V_dir=$TMP_dir&V_mod=$TMP_funclink&funclink=1&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=detail&par0=%%1%%&subfunc=browse',800,600);\"><img src='images/lupa.gif'></a> ".$TMP_value;
					    }
					    ...
					    $TMP_value.=$TMP_arr[$TMP_cont];
				    	    ...
				    	    if ($value!="" && $TMP_value!="" && $TMP_value!="&nbsp;" && $TMP_value!="_") $TMP_linkvalue="<a href=\"javascript:RAD_OpenW('$PHP_SELF?V_dir=$TMP_dir&V_mod=$TMP_funclink&funclink=1&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=detail&par0=".urlencode($value)."&subfunc=browse',800,600);\"><img src='images/lupa.gif'></a>";
					}
				}
				if ($TMP_linkvalue!="") {
					$TMP_brline[$numrow].=$htmllinkend.$htmlEditEnd.$TMP_linkvalue.$htmlEdit;
					if ($htmllink!="") {
						if ($subbrowseSID!="") $TMP_brline[$numrow].=$htmllink."&V_lap=".urlencode($fields[$i]->overlap)."\",$RAD_width,$RAD_height);'>";
						else $TMP_brline[$numrow].=$htmllink."&V_lap=".urlencode($fields[$i]->overlap)."\">";
					}
...
//----- fin calcula enlace a funclink
			...
			//    $TMP_brline[$numrow].=$val;
 			...
 			$TMP_brline[$numrow].=$htmllinkend.$htmlEditEnd." </TD>\n";
			$TMP_linkvalue="";
                    } else {
...
//			if ($subfunc=="report" && ${"browsefield".$i}!="")
			if ($subfunc=="report" && $fields[$i]->browsable)
	     	      	echo "<TD $RAD_classrow><b>".RAD_numero(${"Ttotalfield".$i})."</b></TD>";
           	    }
		    echo "</TR>";
		}
...
//	    if ($func!="browsecalendar" && ($prevpage!=""||$nextpage!="")) echo "<TR><TD><TABLE cellpadding=0 cellspacing=0 class=menu><TR>".RAD_menu_on("$prevpage &nbsp;&nbsp;&nbsp; $nextpage")."</TD></TR></TABLE></TR>";
	    if ($func!="browsecalendar") echo "</tbody></table>";
	    if (($subfunc == "list")||($subfunc == "report")||($func == "search" && $showall != "")) {
		...
		if ($subbrowseSID!='') {
			...
			$TMP_litunder='<div data-role="controlgroup" data-type="horizontal">'.str_replace(' data-iconpos="notext"','',$prevpage).str_replace('data-iconpos="notext"','data-iconpos="right"',$nextpage).'</div>';
		}
		echo $TMP_litunder;
	    }
		...
		if (empty($RAD_browseditbuttons)){
			...
			else echo "onClick='javascript:saveregs(this);' ";
		...
		echo "\n<div style='text-align:center;padding:2;'><INPUT TYPE=BUTTON onClick='javascript:deleregs();' class=button NAME=Delete ACCESSKEY='S' TITLE='ALT+D' VALUE="._DEF_NLSDeleteString.">&nbsp;&nbsp;<INPUT TYPE=RESET class=button  ACCESSKEY='R' TITLE='ALT+R' VALUE="._DEF_NLSClearAll."></div><INPUT TYPE=HIDDEN NAME=V_numrows VALUE='$numrow'></FORM>\n";
