...
//---------------------------------------------------------------------------
	...
	if ($func == "search" && $RAD_sql_pass!=true) {
		...
		for ($i = 0; $i < $MaxSearchFields; $i++) {
			...
			$operator=${"operator".$i};
			if (substr($fields[$findex[$TMP_searchfield]]->type,0,4)=="date") {
			   if ($searchvalue=="" && ${${"searchfield".$i}."_date"}!="") {
				$searchvalue=substr(${${"searchfield".$i}."_date"},4,4)."-".substr(${${"searchfield".$i}."_date"},2,2)."-".substr(${${"searchfield".$i}."_date"},0,2);
			   }
			   if ($searchvalue=="" && ${${"searchfield".$i}."_date5"}!="") {
				$searchvalue=substr(${${"searchfield".$i}."_date5"},0,4)."-".substr(${${"searchfield".$i}."_date5"},5,2)."-".substr(${${"searchfield".$i}."_date5"},8,2);
			   }
			   if ($searchvalueTO=="" && ${${"searchfield".$i}."TO_date"}!="") {
				$searchvalueTO=substr(${${"searchfield".$i}."TO_date"},4,4)."-".substr(${${"searchfield".$i}."TO_date"},2,2)."-".substr(${${"searchfield".$i}."TO_date"},0,2);
			   }
			   if ($searchvalueTO=="" && ${${"searchfield".$i}."TO_date5"}!="") {
				$searchvalueTO=substr(${${"searchfield".$i}."TO_date5"},0,4)."-".substr(${${"searchfield".$i}."TO_date5"},5,2)."-".substr(${${"searchfield".$i}."TO_date5"},8,2);
			   }
			   if (strlen($searchvalue)==8 && ${${"searchfield".$i}."_date"}=="" && ${${"searchfield".$i}."_date5"}=="") {
				$searchvalue=substr($searchvalue,4,4)."-".substr($searchvalue,2,2)."-".substr($searchvalue,0,2);
			   }
			   if (strlen($searchvalueTO)==8 && ${${"searchfield".$i}."_date"}=="" && ${${"searchfield".$i}."_date5"}=="") {
				$searchvalueTO=substr($searchvalueTO,4,4)."-".substr($searchvalueTO,2,2)."-".substr($searchvalueTO,0,2);
			   }
			   if ($searchvalue=="--"||$searchvalue=="--00") $searchvalue="";
			   if ($searchvalueTO=="--"||$searchvalueTO=="--00") $searchvalueTO="";
			   $A_x=array();
			   if (ereg("-",$searchvalue)) $A_x=explode("-",$searchvalue);
			   if (ereg("/",$searchvalue)) $A_x=explode("/",$searchvalue);
			   if (ereg(" ",$searchvalue)) $A_x=explode(" ",$searchvalue);
			   if (count($A_x)==3 && strlen($A_x[2])==4) $searchvalue=$A_x[2]."-".$A_x[1]."-".$A_x[0]; // gira la fecha
			}
...
//			if ($TMP_searchfield!="" && $operator!="") setSessionVar("SESSION_srch_".$TMP_searchfield,$searchvalue,0);
			...
			$TMP_type=$fields[$findex[$TMP_searchfield]]->type;
			$TMP_dtype=$fields[$findex[$TMP_searchfield]]->dtype;
			if (($TMP_dtype=="plistm" || $TMP_dtype=="plistdbm" || $TMP_dtype=="plistdbmtree" || 
			    $TMP_dtype=="popupdbm" || $TMP_dtype=="rlistm" || $TMP_dtype=="rlistdbm" || $TMP_dtype=="checkboxm" || $TMP_dtype=="checkboxdbm") &&
			   ...
			   if (substr($TMP_dtype,0,3)=="num") $searchvalue=RAD_str2num($searchvalue);
			   if (ereg("'",$searchvalue)) {
			   ...
			   if ($operator=="BEGIN") {
				...
				if ($defaultfilter!="") $defaultfilter .= " AND ";
				if ($operator=="=") {
					if ($searchvalue!="") $defaultfilter .= $V_tablename.".".$TMP_searchfield."=".converttosql($searchvalue);
					else if ($TMP_type=="date") $defaultfilter .= "(".$V_tablename.".".$TMP_searchfield."='' OR ".$V_tablename.".".$TMP_searchfield." is NULL or ".$V_tablename.".".$TMP_searchfield." like '0%')";
					else $defaultfilter .= "(".$V_tablename.".".$TMP_searchfield."='' OR ".$V_tablename.".".$TMP_searchfield." is NULL)";
				} else if (ereg("LIKE",$operator) && (_DEF_dbtype=="Oracle")) $defaultfilter .= " UPPER($V_tablename.$TMP_searchfield)";
					...
					case ("NOT IN"):
						$defaultfilter .= " $operator ($searchvalue)";
						//if (trim($searchvalue)!="") $defaultfilter .= " $operator ($searchvalue)";
						//else $defaultfilter .= " $operator (0)";
						break;
					case ("LIKE"):
					case ("NOT LIKE"): {
...
//			$searchvalue=${"searchvalue".$i};
	...
	if ($start>0) { 
	    ...
	    $jumps["start"]=0;
	    if (($start-10*$limit)>-1) {
	        $prevpage .= "<A ACCESSKEY='2' TITLE='ALT+2 -5 "._DEF_NLSPage."s' HREF=\"$url".($start-10*$limit).$tabURLROI."\">"._DEF_NLSPageBefore."</A>";
		$jumps["prevm"]=($start-10*$limit);
	    } else {
	        if (($start-5*$limit)>-1) {
		    $prevpage .= "<A ACCESSKEY='2' TITLE='ALT+2 -5 "._DEF_NLSPage."s' HREF=\"$url".($start-5*$limit).$tabURLROI."\">"._DEF_NLSPageBefore."</A>";
		    $jumps["prevm"]=($start-5*$limit);
		}
	    }
	    $tmp = max($start-$limit,0);
	    if ($tmp>0) {
		$prevpage .= "<A ACCESSKEY='3' TITLE='ALT+3 "._DEF_NLSPrevious." "._DEF_NLSPage."' HREF=\"$url".$tmp.$tabURLROI."\">"._DEF_NLSBefore." </A>";
		$jumps["prev"]=$tmp;
	    }
	}
	if ($start + $limit < $found) { 
	    $tmp = $start+$limit*2;
	    if ($tmp<$found) {
		$nextpage = "<A ACCESSKEY='4' TITLE='ALT+4 "._DEF_NLSNext." "._DEF_NLSPage."' HREF=\"$url".($start+$limit).$tabURLROI."\">"._DEF_NLSAfter."</A>";
		$jumps["next"]=$start+$limit;
	    }
	    if (($start+10*$limit)<$found) {
	        $nextpage .= "<A ACCESSKEY='5' TITLE='ALT+5 +5 "._DEF_NLSPage."s' HREF=\"$url".($start+10*$limit).$tabURLROI."\">"._DEF_NLSPageAfter."</A>";
		$jumps["nextm"]=$start+10*$limit;
	    } else {
	        if (($start+5*$limit)<$found) {
		    $nextpage .= "<A ACCESSKEY='5' TITLE='ALT+5 +5 "._DEF_NLSPage."s' HREF=\"$url".($start+5*$limit).$tabURLROI."\">"._DEF_NLSPageAfter."</A>";
		    $jumps["nextm"]=$start+5*$limit;
		}
	    }
	    if ($found>($start+2*$limit)) { 
		$nextpage .= "<A ACCESSKEY='6' TITLE='ALT+6' HREF=\"$url".($found-$limit).$tabURLROI."\">"._DEF_NLSEnd."</A>"; 
		$jumps["end"]=$found-$limit;
	    } else if ($found>($start+$limit)) { 
		$nextpage .= "<A ACCESSKEY='6' TITLE='ALT+6' HREF=\"$url".($start+$limit).$tabURLROI."\">"._DEF_NLSEnd."</A>";
