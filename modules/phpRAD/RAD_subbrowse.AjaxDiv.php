...
$TMP="";
global $subbrowseSID,$PHPSESSID, $RAD_subbrowseCont, $PHP_SELF, $HTTP_SESSION_VARS;
//---------------------------------------------------------------------------
//------------------------- SubBrowse
//---------------------------------------------------------------------------
...
$TMP_DEF_URL=_DEF_URL;
$TMP_New="";
if ($func=="edit" || $func=="new" || $func=="detail") {
	if ($fields[$i]->browsedit && $subfunc!="list") {
    	    $TMP_URLfileNew=$TMP_DEF_URL."index.php?V_dir=$TMP_dir&V_mod=".$TMP_file."&V_prevdir=$V_dir&V_prevmod=$V_mod&menuoff=x&headeroff=x&footeroff=x&func=new&subfunc=browse&V_roi=".urlencode($TMP_field1."='".$TMP_id."'").$SESSION_SID.$TMP_V_roi2;
    	    $TMP_URLfileNew=$PHP_SELF."?V_dir=$TMP_dir&V_mod=".$TMP_file."&V_prevdir=$V_dir&V_prevmod=$V_mod&menuoff=x&headeroff=x&footeroff=x&func=new&subfunc=browse&V_roi=".urlencode($TMP_field1."='".$TMP_id."'").$SESSION_SID.$TMP_V_roi2;
...
$TMP_URLfile=$TMP_DEF_URL."index.php?V_dir=$TMP_dir&V_mod=".$TMP_file."&V_prevdir=$V_dir&V_prevmod=$V_mod&V_lap=".urlencode($V_lap)."&menuoff=x&headeroff=x&footeroff=x&subbrowseSID=".$PHPSESSID."&numsubbrowse=$RAD_subbrowseCont&func=".$TMP_func."&RAD_width=".$TMP_width."&RAD_height=".$TMP_height."&V_roi=".urlencode($TMP_field1."='".$TMP_id."'").$TMP_V_roi2;
...
$TMP_content = '<div id="ajax"></div>
<script type="text/javascript">
	window.onload=function(){
		RAD_ajaxIncludeURL("'.$TMP_URLfile.'","ajax")
	}
</script>
';
...
$TMP.= $TMP_content;
$TMP_content="";
if (eregi("PDA",$HTTP_SESSION_VARS["SESSION_theme"])) $TMP = eregi_replace(" target=_blank","",$TMP);
//--------------------------------------------------------------------------------------------------------------------------------------------------
