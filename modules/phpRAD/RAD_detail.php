...
//				echo "\n<script>\nsalto=history.length-1;\nhistory.go(-salto);\n</script>\n";
			...
			if ($TMP_overlap && count($TMP_overlaptext)>0) {
				...
				if ($text=="") $text=" ";
				$TMP_contVlap++;
				if ($TMP_contVlap==_DEF_LapBreak) echo "</tr></table><table cellpadding=0 cellspacing=0 class=submenu><tr>";
			    ...
			    //if ($RAD_printbarcode!="" && strtolower($lapoff)!="x") echo RAD_submenu_off("<a title='Imprime Codigo de Barras de Registro' class=submenuoff href='javascript:RAD_OpenW(\"".$PHP_SELF."?par0=$par0&dbname=$dbname&V_dir=$V_dir&V_idmod=$V_idmod&func=detail&subfunc=barcode&blocksoff=x&menuoff=x&headeroff=x&footeroff=x\",650,460);'><img border=0 style='vertical-align:middle;' src='images/print.png'> $par0 </a>\n");
			    if ($RAD_printbarcode!="" && strtolower($lapoff)!="x") echo RAD_submenu_off("<a title='Imprime Codigo de Barras de Registro' class=submenuoff href='javascript:RAD_OpenW(\"".$PHP_SELF."?par0=$par0&dbname=$dbname&V_dir=$V_dir&V_idmod=$V_idmod&func=detail&subfunc=barcode&blocksoff=x&menuoff=x&headeroff=x&footeroff=x\",650,460);'><font style='font-weight:bold; font-size:1.5em;'>&#9288;</font> $par0 </a>\n");
			    echo "</tr></table>\n";
			}
...
//					$htmlEdit="<A HREF=\"".$PHP_SELF."?func=edit&dbname=$dbname&menuoff=x&footeroff=x&headeroff=x&fieldedit=".$fields[$i]->name.$urlidnames.$URLROI."\">"._DEF_NLSBrowseEdit." ";
				...
				if ($fields[$i]->dtype == "function") {
					...
					$A_TMP_funclink=explode(" ",trim($TMP_funclink));
					if (count($A_TMP_funclink)>1) { $TMP_funclink=$A_TMP_funclink[0]; $TMP_params=$A_TMP_funclink[1]; }
					else { $TMP_params=""; }
					if ($TMP_params!="") { //params para campo no multiple
						$A_TMP_params=explode(",",$TMP_params.",");
						$TMP_params=""; $TMP_contparam=0; 
						foreach($A_TMP_params as $TMP_idx=>$TMP_fieldparam) {
							$TMP_params.="&par".$TMP_contparam."=".urlencode($db->Record["".$TMP_fieldparam]);
							$TMP_contparam++; 
						}
					} else {
						$TMP_params="&par0=".urlencode($value);
					}
					if ($TMP_funclink!="") if (is_modulepermitted("", $TMP_dir, $TMP_funclink) || is_admin()) {
						if (in_array($fields[$i]->dtype,array("plistm","plistdbm","plistdbmtree","popupdbm","checkboxm","checkboxdbm"))){
						    ...
						    $TMP.=$TMP_arr[$TMP_cont];
					    	    ...
					    	    if ($value!="") $TMP_linkvalue="<a href=\"javascript:RAD_OpenW('$PHP_SELF?V_dir=$TMP_dir&V_mod=$TMP_funclink&funclink=1&dbname=$dbname&headeroff=X&footeroff=X&menuoff=&func=detail".$TMP_params."&subfunc=browse',1,1);\">";
						}
					}
