...
";
	...
	echo "<CENTER><FORM autocomplete=off ACTION=$PHP_SELF";
	...
	for ($i = $inifield; $i < $lastfield+1; $i++) {
		...
		if (($func == "new" && $fields[$i]->nonew==true) || ($func == "edit" && $fields[$i]->noedit==true)) {
			...
			if ($fields[$i] -> readonly == false) echo "<INPUT TYPE=HIDDEN NAME=V0_".$fields[$i]->name." VALUE='".htmlspecialchars($fvalue)."'>\n";
...
//					if ($fields[$i] -> canbenull) echo "<BR>(",_DEF_NLSOptional.")";
			...
			if ($fields[$i]->dtype == "function") {
				...
				if ($fields[$i] -> readonly == true) {
					$TMP=RAD_showfield($fields[$i]->dtype, $fields[$i]->extra, $fvalue)."<INPUT TYPE=HIDDEN NAME=V0_".$fields[$i]->name." VALUE='".htmlspecialchars($fvalue)."'></span><span id='HIDV0_".$fields[$i]->name."'></span>";
					if ($fields[$i]->coledit!="") $TMP_div.= $TMP;
	    				else echo $TMP;
				} else {
	...
	if ($RAD_editbuttons == ""){
		...
		echo "<CENTER><INPUT TYPE=BUTTON onClick='RAD_checkFields(this);' class=button NAME=Save ACCESSKEY='S' TITLE='ALT+S' VALUE='"._DEF_NLSSave."' id='RAD_SAVEBUTTON'>\n";
		if ($RAD_buttonDuplicate!="") echo " &nbsp; &nbsp; <INPUT ACCESSKEY='X' TITLE='ALT+X' TYPE=BUTTON OnClick='javascript:document.F.func.value=\"insert\";RAD_checkFields();' class=button NAME=Duplicate ACCESSKEY='X' TITLE='ALT+X' VALUE='"._DEF_NLSDuplicate."'>\n";
		...
		echo " &nbsp; &nbsp; <INPUT ACCESSKEY='R' TITLE='ALT+R' TYPE=BUTTON OnClick=\"javascript:document.location=document.location.href+'&reset=x'\" class=button VALUE='"._DEF_NLSClearAll."'></CENTER>";
