...
/*
            if (in_array('modulo', $TMP_campo)) {
                $TMP_where.=" AND modulo='".$V_mod."'";
            }
*/
            if (in_array('usuario', $TMP_campo)) {
                $TMP_user=base64_decode($HTTP_SESSION_VARS["SESSION_user"]);
                $TMP_where.=" AND usuario='".$TMP_user."'";
            }

			$TMP_URL=$PHP_SELF."?";
			foreach ($_REQUEST as $key=>$val) $TMP_URL.=$key."=".urlencode($val)."&";
			/*
			//$TMP_cmd="SELECT * FROM impresos WHERE (tabla='$V_tablename' OR tabla='$V_mod') AND (url IS NOT NULL AND url!='')";
			$TMP_cmd="SELECT * FROM impresos WHERE modulo='$V_mod' OR (modulo IS NULL AND tabla='".$V_tablename."')";
            if ($TMP_where!="") $TMP_cmd.=$TMP_where;
 	...
 	echo "<FORM autocomplete=off ACTION=$PHP_SELF METHOD=GET NAME=F>$FORMROI\n";
